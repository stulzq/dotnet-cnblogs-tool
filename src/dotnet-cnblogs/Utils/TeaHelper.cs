...
namespace Dotnetcnblog.Utils
{
    ...
    public class TeaHelper
    {
        ...
        public static byte[] Encrypt(byte[] data, byte[] key)
        {
            ...
            var result = new byte[dataBytes.Length * 4];
        ...
        public static byte[] Decrypt(byte[] data, byte[] key)
        {
            ...
            {
                var result = new byte[dataBytes.Length - 1];
        ...
        private static uint[] FormatKey(byte[] key)
        {
            ...
            if (key.Length < 16)
            {
                ...
                for (var k = key.Length; k < 16; k++) refineKey[k] = 0x20;
            }
            else
            {
        ...
        private static void Encrypt(uint[] v, uint[] k)
        {
            var y = v[0];
            var z = v[1];
            uint sum = 0;
